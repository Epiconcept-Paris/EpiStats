*! Stata Journal version 1.0.1  Ben Jann  09apr2008
...
// prepare f (observed counts) and h (expected counts)
    ...
    if (st_local("h0exp")!="") {
        ...
        for (i=1;i<=rows(levels);i++) {
            ...
            if (SJmgof_isconstant(tmp)==0) _error(3498,
...
// compute stats
    ...
    else if (method=="mc")  m_arg = strtoreal(st_local("reps"))
    else                    m_arg = .
    lambda = strtoreal(st_local("cr2"))
    res = SJmgof(f, h, method, stats, lambda, m_arg, st_local("dots")=="", comp)

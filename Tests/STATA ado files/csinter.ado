...
if `_total' == 0 {
   display as error "No record selected"
   exit
}

markout `touse' `1' `2' `by'
quietly : count if `touse'
local _missing = `_total' - r(N) 
...
local _Tot = `_Totexp'+`_Totunexp'
if `byvalue'==1 display in text %15s "`by' = Exposed (`_Tot')" 
if `byvalue'==0 display in text %15s "`by' = Unexposed (`_Tot')"
...
display
display "Missing values : " `_missing'  ///
    " (" %5.2f (`_missing')/`_total'*100  "%)"
